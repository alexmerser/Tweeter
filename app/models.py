...
class Timeline:
	def page(self, page):
		_from = (page-1)*10
		_to = (page)*10
		# Returns 10 posts from that page on the timeline from the Redis DB
		return [Post(post_id) for post_ids in r.lrange('timeline', _from, _to)]


# Setters and Getters for Models and the Redis DB
class Model(object):
...
class Post(Model):
	...
	def create(user, content):
		...
		# add post to timeline of all followers
		for follower in user.followers:
			follower.add_timeline_post(post)

		# search for user mentions in content and add to Redis
		mentions = re.findall('@\w+', content) # regex for @ followed by word characters
